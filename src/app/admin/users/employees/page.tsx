...
import { useAdminEmployees } from '@saubio/utils';
...
export default function AdminEmployeesPage() {
  ...
  const employeesQuery = useAdminEmployees({ page, pageSize: 20, status, search });
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-2">
        ...
        <h1 className="text-3xl font-semibold text-saubio-forest">Employés</h1>
        <p className="text-sm text-saubio-slate/70">
          Comptes internes (Ops, support, finance). Vérifiez rapidement les accès actifs.
        </p>
      </header>
            ...
            <tbody>
              {isLoading && !data
                ? Array.from({ length: 5 }).map((_, index) => (
                    <tr key={`employees-skeleton-${index}`} className="border-b border-saubio-forest/5">
              ...
              {data?.items.map((employee) => (
                <tr key={employee.id} className="border-b border-saubio-forest/5 last:border-none">
                  <td className="px-3 py-2 font-semibold text-saubio-forest">{employee.name}</td>
                  <td className="px-3 py-2">{employee.email}</td>
                  <td className="px-3 py-2">{employee.role}</td>
                  <td className="px-3 py-2">{dateFormatter.format(new Date(employee.createdAt))}</td>
                  <td className="px-3 py-2">{employee.lastLoginAt ? dateFormatter.format(new Date(employee.lastLoginAt)) : 'Jamais'}</td>
                  <td className="px-3 py-2">
                    <span className={`inline-flex rounded-full border px-2 py-1 text-xs font-semibold ${statusBadge(employee.status)}`}>
